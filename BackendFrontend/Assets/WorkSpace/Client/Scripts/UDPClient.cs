...
public class UDPClient : SingletonMonoBehaviour<UDPClient>
{
    ...
    public UnityEvent<string> OnIPEndPointReceivedEvent { get; private set; } = new UnityEvent<string>();
    public UnityEvent OnGameStartEvent { get; private set; } = new UnityEvent();
    public UnityEvent OnGameExitEvent { get; private set; } = new UnityEvent();
    public int HostOrder { get => hostOrder; set => hostOrder = value; }
    ...
    private void LinkEvent()
    {
        ...
        TCPClient.Instance.OnMatchFoundEvent.AddListener(StartClient);

        // 매칭 성공 시, 게임 시작
        TCPClient.Instance.OnMatchSucceedEvent.AddListener(StartGame);
    }
    void StartClient()
    {
        // 제거
